...
class TagUpdateScreen extends StatefulWidget {
  // const TagCreateScreen({ Key? key }) : super(key: key);

  @override
  _TagUpdateScreen createState() => _TagUpdateScreen();
}

class _TagUpdateScreen extends State<TagUpdateScreen> {
  ...
  void initState() {
    super.initState();
    getUser();
    tagToUpdate = getTagToUpdate();
  }

  getTagToUpdate(){
    Map<String, dynamic> item = myStorage.getItem('tagToUpdate');
    if (item == null) {
        return null;
      }
    Tag myTag = Tag.fromJson(item);
    print(myTag.tagName);
    setState(() {
      
          myTagNameController.text = myTag.tagName;
          dropdownValueTag = myTag.tagColor;
  ...
  updateTag() async {
    ...
    setState(() {
      myUserId = prefs.get('userId');
      url = Uri.https('10.0.2.2:5001', '/tags/${tagToUpdate.id}');
    });
    try {
